...
The notebook downloads its own NLTK resources (`punkt_tab`, `stopwords`, `wordnet`, `averaged_perceptron_tagger_eng`) on first run.

## Possible Next Steps

- Error analysis on misclassified reviews to identify failure patterns (e.g. sarcasm, mixed sentiment)
- Try `ngram_range=(1,2)` in TF-IDF to capture bigram phrases like "not good"
- Hyperparameter tuning via `GridSearchCV`
- Compare against a fine-tuned transformer model (e.g. DistilBERT) as an upper-bound baseline
